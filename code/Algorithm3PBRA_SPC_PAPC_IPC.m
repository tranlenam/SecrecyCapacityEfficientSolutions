function [X,SecrecyCapacity,SecrecyRateSeq] = Algorithm3PBRA_SPC_PAPC_IPC(Hb,He,P0,PAPC,IPC,maxIter)
[nB,nA] = size(Hb);
nE = size(He,1);
K = eye(nB+nE);
H  = [Hb;He];
...
F = [S>=0,real(trace(S))<=P0,real(diag(S))<=PAPC,...
    real(trace(IPC{1,1}*S))<=IPC{1,2}, real(trace(IPC{2,1}*S))<=IPC{2,2}];
for iIter = 1:maxIter
